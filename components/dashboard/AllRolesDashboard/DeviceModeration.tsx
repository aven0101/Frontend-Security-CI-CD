...
export default function DeviceModeration() {
  // Mock stats (replace with API data later)
  const stats = {
    totalDevices: 87,
    activeDevices: 65,
    suspended: 3,
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-[0px_0px_7.22px_0px_rgba(0,0,0,0.15)] overflow-hidden">
            ...
            <div className="flex items-center justify-center gap-2 py-4 border-t border-gray-100">
              ...
              {[1, 2, 3, 4].map(n => (
                <button
                  key={n}
                  className={`w-7 h-7 rounded border text-sm ${n === 2 ? 'bg-[#98C1A9] text-white border-[#98C1A9]' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >{n}</button>
              ))}
              <span className="text-gray-400">&gt;</span>
            </div>

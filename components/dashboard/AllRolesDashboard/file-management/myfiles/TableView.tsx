...
function formatSize(kb: number) {
    ...
    if (kb >= 1024 * 1024 / 1024) return `${(kb / (1024 * 1024 / 1024)).toFixed(2)} GB`;
    if (kb >= 1024) return `${(kb / 1024).toFixed(1)} MB`;
    if (kb >= 1) return `${kb} KB`;
    return `${Math.round(kb * 1024)} B`;
}

export default function FileManagementMyFilesTableView() {
    ...
    const isAllSelected = allIdsOnPage.length > 0 && allIdsOnPage.every((id) => selected.has(id));
    const isSomeSelected = !isAllSelected && allIdsOnPage.some((id) => selected.has(id));
        ...
        <div className="space-y-4">
            ...
            {selected.size > 0 && (
                <div className="flex items-center gap-4 rounded-xl border border-[#97C1A9] bg-white px-3 py-2 text-[#2E3A33]">
                    ...
                    <button className="rounded-md p-1 text-[#6B7A73] hover:bg-[#EDF7F2]" aria-label="Get link">
            ...
            <div className="overflow-hidden rounded-2xl border border-[#97C1A9] bg-white">
                ...
                <div className="flex items-center bg-[#6FAE90] text-white">
                    ...
                    <HeaderCell label="File Name" active={sortKey === 'name'} order={sortOrder} onClick={() => onHeaderSort('name')} className="flex-1" />
                    <HeaderCell label="File Size" active={sortKey === 'sizeKB'} order={sortOrder} onClick={() => onHeaderSort('sizeKB')} className="w-40" />
                    <HeaderCell label="Last Modified" active={sortKey === 'modified'} order={sortOrder} onClick={() => onHeaderSort('modified')} className="w-48" />
            ...
            <ShareLinkDialog
                open={!!shareFile}
                file={shareFile}
                link={shareFile ? `https://yourapp.com/shared/file/${shareFile.id}` : ""}
                copied={copied}
                onCopy={() => {
                    if (!shareFile) return;
...
function HeaderCell({ label, active, order, onClick, className = "" }: { label: string; active?: boolean; order?: 'asc' | 'desc'; onClick?: () => void; className?: string }) {
    return (
        <button onClick={onClick} className={`px-3 py-3 text-left text-sm font-medium flex items-center gap-2 ${className}`}>
            <span>{label}</span>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${active ? 'opacity-100' : 'opacity-60'}`}>
                <path d="M12 5L7 10H17L12 5Z" fill="currentColor" opacity={order === 'asc' ? 1 : 0.35} />
                <path d="M12 19L17 14H7L12 19Z" fill="currentColor" opacity={order === 'desc' ? 1 : 0.35} />
            </svg>
        </button>
    );
}

function Row({ file, checked, onToggle, onGetLink }: { file: FileItem; checked: boolean; onToggle: () => void; onGetLink: () => void }) {
    return (
        <div className="flex items-center">
            <div className="w-12 flex items-center justify-center py-3">
                <input type="checkbox" checked={checked} onChange={onToggle} className="h-4 w-4 rounded-full text-[#2E3A33] focus:ring-0" />
            </div>
            <div className="flex-1 px-3 py-3">
                <div className="flex items-center gap-3">
                    <div className="h-7 w-7 rounded-md border border-[#97C1A9]/60 flex items-center justify-center text-[#6B7A73]">
                    ...
                    {/* Optional star on some rows for demo */}
                    {parseInt(file.id) % 3 === 0 && <StarIcon className="h-4 w-4 text-[#6FAE90]" />}
                </div>
            </div>
            <div className="w-40 px-3 py-3 text-sm text-[#2E3A33]">{formatSize(file.sizeKB)}</div>
            <div className="w-48 px-3 py-3 text-sm text-[#2E3A33]">{file.modified.replace(/\//g, '/')}</div>
            <div className="w-40 px-3 py-3">
                <div className="relative flex items-center gap-2 text-[#6B7A73]">
                    <button onClick={onGetLink} className="rounded-md p-1 hover:bg-[#EDF7F2]" aria-label="Get link">
                        <LinkIcon className="h-4 w-4" />
                    </button>
                    <button className="rounded-md p-1 hover:bg-[#EDF7F2]" aria-label="Share">
                        <ShareIcon className="h-4 w-4" />
                    </button>
                    <button className="rounded-md p-1 hover:bg-[#EDF7F2]" aria-label="Download">
                        <ArrowDownTrayIcon className="h-4 w-4" />
                    </button>
                    <Menu as="div" className="inline-block text-left">
                        <Menu.Button className="rounded-md p-1 hover:bg-[#EDF7F2]" aria-label="More">
                            <EllipsisVerticalIcon className="h-5 w-5" />
                        </Menu.Button>
...
function CommonMenuItems({ className = "absolute right-0 mt-2 w-44 origin-top-right" }: { className?: string }) {
    const items = [
    ...
    return (
    <Menu.Items className={`${className} z-50 rounded-xl border border-[#97C1A9] bg-white p-1 shadow-lg focus:outline-none max-h-[70vh] overflow-auto`}
                    >
...
function ShareLinkDialog({ open, file, link, copied, onCopy, onClose }: ShareLinkDialogProps) {
    return (
                                ...
                                <Dialog.Title className="text-sm text-[#2E3A33]">
                                    ...
                                    {file && (
                                        <>
                                            ...
                                            <span className="text-[#6FAE90]">File: {file.name}.jpg</span>
                                            <span className="ml-1 text-[#8AA097]">]</span>
                                        </>
                                    )}
                                </Dialog.Title>
                                <button onClick={onClose} aria-label="Close" className="rounded-md p-1 text-[#6B7A73] hover:bg-[#EDF7F2]">
                                    <XMarkIcon className="h-5 w-5" />
                                </button>
                            </div>
                            ...
                            <div className="px-4 py-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    >
                                        {copied ? (
                                            <>
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4"><path d="M20 6L9 17l-5-5"/></svg>
                                                Copied
                                            </>
                                        ) : (
                                            <>
